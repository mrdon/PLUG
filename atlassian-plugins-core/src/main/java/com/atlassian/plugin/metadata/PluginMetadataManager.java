...
public interface PluginMetadataManager
{
     ...
     * application. If any {@link ModuleDescriptor}'s are not optional then the
     * plugin is also not optional.
     * 
     * @param plugin used to determine the state, not null.
     * @return true if the plugin can safely be disabled, false if the plugin
     *         being disabled would adversely effect the host application.
     */
    boolean isOptional(Plugin plugin);

    /**
     * This is used to determine if a module is considered optional. If an
     * optional module is disabled it should not adversely effect the host
     * application. A module can not be optional if its containing plugin is not
     * optional.
     * 
     * @param moduleDescriptor used to determine state, not null.
     * @return true if the module can safely be disabled, false if the module
     *         being disabled would adversely effect the host application.
     */
