...
public interface ModuleDescriptor<T> extends Resourced
{
     ...
     *
     * <p> Since atlassian-plugins v2.2, you can no longer load classes from the plugin in this method, because the OSGi
     * bundle that they will live in is not built yet.
     * Load classes in the {@link com.atlassian.plugin.descriptors.AbstractModuleDescriptor#enabled()} method instead.
     * 
     * @param plugin The plugin that the module belongs to.
     * @param element XML element representing the module.
     * @throws PluginParseException Can be thrown if an error occurs while parsing the XML element.
     */
     ...
     * @return {@code true} if this plugin module is a "system" plugin that shouldn't be made visible/disableable to the user.
     */
     ...
     * @param plugin
     * TODO: The plugin parameter is redundant. The ModuleDescriptor must know its parent plugin in order to implement getPlugin()
     */
