...
public class VersionStringComparator implements Comparator
{
    ...
    public int compare(String version1, String version2)
    {
        ...
        if (!thisVersion.matches(VALID_VERSION_PATTERN) || !compareVersion.matches(VALID_VERSION_PATTERN))
        {
            throw new IllegalArgumentException(
                "Version number '" + thisVersion + "' cannot be compared to '" + compareVersion + "'");
        }
        ...
        Comparator componentComparator = new VersionStringComponentComparator();
    ...
    private class VersionStringComponentComparator implements Comparator
    {
        ...
        {
            if (component1.equalsIgnoreCase(component2)) return 0;

            if (isInteger(component1) && isInteger(component2))
            {
                // both numbers -- parse and compare
                if (Integer.parseInt(component1) > Integer.parseInt(component2)) return FIRST_GREATER;
                if (Integer.parseInt(component2) > Integer.parseInt(component1)) return SECOND_GREATER;
                return 0;
            }

            // 2.3-alpha < 2.3.0
            if ("0".equals(component1)) return FIRST_GREATER;
            if ("0".equals(component2)) return SECOND_GREATER;

            // 2.3a < 2.3
            if (isInteger(component1) && component2.startsWith(component1)) return FIRST_GREATER;
            if (isInteger(component2) && component1.startsWith(component2)) return SECOND_GREATER;
