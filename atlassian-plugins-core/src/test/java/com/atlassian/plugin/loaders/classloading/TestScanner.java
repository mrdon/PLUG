...
public class TestScanner extends AbstractTestClassLoader
{
    public void testNormalOperation() throws Exception
    {
        ...
        final Scanner scanner = new Scanner(pluginsDirectory);
        scanner.scan();
        final Collection deployedUnits = scanner.getDeploymentUnits();
        assertEquals(2, deployedUnits.size());

        // put them into a list so we're sure we get them in the right order
        final Iterator iterator = deployedUnits.iterator();
        DeploymentUnit unit;
    ...
    {
        final File pluginsDirectory = getPluginsDirectory();
        final File paddington = new File(pluginsDirectory, "paddington-test-plugin.jar");
        final File duplicate = new File(pluginsDirectory, "duplicate-test-plugin.jar");
    ...
    public void testModifyJar() throws Exception
    {
        ...
        final File testTempDirectory = new File(System.getProperty("java.io.tmpdir") + File.separator + "TestScannerTests");
        ...
        final File newPaddington = new File(testTempDirectory, "paddington-test-plugin.jar");
        ...
        // should be 2 to start with
        final Scanner scanner = new Scanner(testTempDirectory);
        scanner.scan();
        ...
        // modify the JAR file
        final long secondModification = originalModification + 2000;
    ...
    {
        final Scanner.JarFileFilter filter = new Scanner.JarFileFilter();
